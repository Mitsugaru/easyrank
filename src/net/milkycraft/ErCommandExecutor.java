 ...
 */
public class ErCommandExecutor extends PermissionsCommand implements
        CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command,
        ...
        if (split.length == 2) {
            ...
            if (plugin.getVaultPerms().getPrimaryGroup(target)
    ...
    private void rankOfflinePlayer(CommandSender sender, String[] split) {
        final Player target = plugin.getServer().getOfflinePlayer(split[0])
                .getPlayer();
        // TODO either use these variables or remove them.
    ...
    public void groupUsersAdd(Plugin plugin, CommandSender sender, String[] args) {
        ...
        String worldName = this.autoCompleteWorldName(((Player) sender)
                .getWorld().getName());
        ...
        sender.sendMessage(ChatColor.WHITE + "User " + user.getName()
                + " added to " + groupName + " !");
        this.informPlayer(plugin, userName, "You are assigned to \""
                + groupName + "\" group");
    }
    ...
    private boolean handleConsoleCommand(Command command, String commandLabel,
        ...
        if (split.length == 2) {
            try {
                ...
                if (plugin.getVaultPerms().getPrimaryGroup(target)
                        .equalsIgnoreCase(rank)) {
                    plugin.getLogger().info(carget + " is already a " + rank);
                    return true;
                }
                removeOldRanks(target);
                plugin.getVaultPerms().playerAddGroup(target, rank);
                ...
                if (plugin.getVaultPerms().getPrimaryGroup(target)
                    ...
                    plugin.getLogger().info(
                            "Sucessfully ranked " + carget + " to " + rank);
                    target.sendMessage(ChatColor.GRAY
                ...
                if (Settings.aroolp) {
                    ...
                    removeOldRanks(target);
                    plugin.getVaultPerms().playerAddGroup(target, rank);
                    plugin.getLogger().info("Ranked " + carget + " to " + rank);
                }
            }

        } else {
            plugin.getLogger().info("Use: /rank player group");
            return true;
        }
        return false;
    }
